...
library(ggplot2)
```{r}
if (!exists("climate_data") || nrow(climate_data) == 0) {
  message("Using mock data for plotting.")
...
#Create the static visualisation
library(ggplot2)
...
```{r}
if (exists("plot_summarise") && !is.null(climate_data)) {
  # Animated visualization using plot_summarise
  animation <- plot_summarise(climate_data)
  
  # Display the animation
  animation
} else {
  message("The 'plot_summarise()' function is not available or climate_data is missing. Skipping animated visualisation.")
}
```
